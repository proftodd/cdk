...
public class BioPolymer extends Polymer implements java.io.Serializable, IBioPolymer {
    ...
    /*
     * Could look like this if you ensured individual name giving for ALL
     * monomers: public Monomer getMonomer(String cName) { Enumeration keys =
     * strands.keys(); Monomer oMonomer = null; while(keys.hasMoreElements()) {
     * if
     * (((Strand)strands.get(keys.nextElement())).getMonomers().containsKey(cName
     * )) { Strand oStrand = (Strand)strands.get(keys.nextElement()); oMonomer =
     * oStrand.getMonomer(cName); break; } } return oMonomer; }
     */
