...
public class OverlapResolver {
    ...
    public double displace(IAtomContainer ac, Vector overlappingAtoms, Vector overlappingBonds) {
        ...
        do {
            /*
             * we take a random overlapping pair of atoms
             */
            p = (int) (Math.random() * overlappingAtoms.size());
            logger.debug("Taking overlap pair no. " + p);
            op = (OverlapPair) overlappingAtoms.elementAt(p);
            /*
             * Now we have an overlapping pair of atoms We calculate the 2D
             * vector formed by the positions of both and translate one of the
             * atoms by one tenth of a bond length
             */
            ...
            if (Double.isNaN(v2.x)) v2.x = 0.01;
            if (Double.isNaN(v2.y)) v2.y = 0.01;
            ...
            overlapScore = getOverlapScore(ac, overlappingAtoms, overlappingBonds);
            steps++;
